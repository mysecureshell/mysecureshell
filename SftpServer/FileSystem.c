...
int FSCheckSecurity(const char *fullPath, const char *path)
{
	...
	if (HAS_BIT(gl_var->flagsGlobals, SFTPWHO_HIDE_NO_ACESS))
	{
		...
		{
			if ((st.st_uid == getuid() && (st.st_mode & (S_IRUSR | S_IWUSR | S_IXUSR)) != 0)
					|| (UserIsInThisGroup(st.st_gid) == 1 && (st.st_mode & (S_IRGRP | S_IWGRP | S_IXGRP)) != 0)
					|| (st.st_mode & (S_IROTH | S_IWOTH | S_IXOTH)) != 0)
				result = SSH2_FX_OK;
		}
		if (errno == ENOENT)
			result = SSH2_FX_OK;
