...
try_shm:
	if ((key = ftok(_shmfile, _shmkey)) != -1)
	{
		...
		if ((shmid = shmget(key, sizeof(t_shm), 0)) == -1)
			if (create == 1)
			{
				shmid = shmget(key, sizeof(t_shm), IPC_CREAT | IPC_EXCL | 0622);
				eraze = 1;
			}
...
int SftpWhoCleanBuggedClient()
{
	...
	for (i = 0; i < SFTPWHO_MAXCLIENT; i++)
		...
		{
			//add 60s to make sure that the session is definitively dead
			if ((_sftpwho_ptr[i].time_begin + _sftpwho_ptr[i].time_total + 60) < t)
